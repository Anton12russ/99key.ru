...
<div class="row">
   ...
   <div class="col-md-9">
    ...
    <h1><i class="fa fa-car" aria-hidden="true"></i> <?=$this->title?></h1>
...
<?if ((!isset($get['appliances_0']) && !isset($get['appliances_1']) || (isset($get['appliances_0']) && $get['appliances_0'] == '') ||  (isset($get['appliances_1']) && $get['appliances_1'] == ''))) {?>Водители и пассажиры<?}?>
 ...
 <div class="col-md-6">
  <div class="form-group field-field-name required">
       ...
       <input type="text" class="form-control calendarmain1" placeholder="год-месяц-день"  name="date" value="<?if (isset($get['date'])) { echo $get['date'];}?>"/>
   </div>
</div> 
...
<button id="btn" class="btn btn-success go-search">Искать <span></span> <i class="fa fa-search" aria-hidden="true"></i></button>
   ...
   <? if (Yii::$app->request->get('sort_tyme') == 'DESC') {echo 'Время отправки > дальше';}if (Yii::$app->request->get('sort_tyme') == 'ASC') {echo 'Время отправки > ближе';}if (Yii::$app->request->get('sort') == '' && Yii::$app->request->get('sort_tyme') == '') {echo 'Новые';}?>
     ...
     <ul class="dropdown-menu sort-menu" role="menu" aria-labelledby="dLabel">
        <div class="treug"></div>
        <li><a href="<?=Yii::$app->userFunctions3->urlshop($_SERVER['REQUEST_URI'])?>">Самые новые </a></li>
		<li><a href="<?=Yii::$app->userFunctions3->urlshop($_SERVER['REQUEST_URI'])?>sort_tyme=ASC">Время отрпавки > ближе</a></li>
		<li><a href="<?=Yii::$app->userFunctions3->urlshop($_SERVER['REQUEST_URI'])?>sort_tyme=DESC">Время отправки > дальше</a></li>
        <li><a href="<?=Yii::$app->userFunctions3->urlshop($_SERVER['REQUEST_URI'])?>sort=ASC">От дешевых к дорогим</a></li>
	    <li><a href="<?=Yii::$app->userFunctions3->urlshop($_SERVER['REQUEST_URI'])?>sort=DESC">От дорогих к дешевым</a></li>
