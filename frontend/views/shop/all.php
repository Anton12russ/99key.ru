...
?>
    ...
    <div class="col-md-12">
	  <div class="cat-board-body <?=$nostatus?>">
	    <div class="shop-all-img <? if (isset($this->blocks['right'])){?>col-md-3  col-sm-4<? }else{ ?>col-md-2  col-sm-3<? } ?> col-xs-4" style="background-size: 100%; background-image: url(<? if ($one->img) {?> <?= Yii::getAlias('@shop_logo').'/'.$one->img;?> <? }else{ ?><?= Yii::getAlias('@blog_image').'/'?>no-photo_all.png<? } ?>);"><a class="img_a" href="<?=$url?>"></a></div>
	    <div class="all_body  <? if (isset($this->blocks['right'])){?>col-md-9 col-sm-8<? }else{ ?>col-md-10 col-sm-9<? } ?> col-xs-8">
	    <h3><?=$statususs?><?=$actsuss?><a class="cat-bo-href" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=Yii::$app->userFunctions->substr_user($one->name, 150)?>" href="<?=$url?>"> <?=Yii::$app->userFunctions->substr_user($one->name, 100)?></a></h3>
