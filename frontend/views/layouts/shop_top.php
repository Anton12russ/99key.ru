...
<h3><i class="fa fa-shop" aria-hidden="true"></i><a href="https://1tu.ru/shop?sort=DESC" style="text-decoration: none;"> Популярные магазины</a></h3>
...
<?$shop_url = PROTOCOL.$one->domen.'.'.DOMAIN;?>
  ...
  <?if(isset($one['rayting']) && $one['rayting'] > 0) {$rat = 'green';}else{$rat = '';}if(isset($one['rayting']) && $one['rayting'] < 0) {$rat = 'red';}if(!isset($one['rayting'])) {$rat = 'gray';}?>
     ...
     <a class="href-shop-top" href="<?=$shop_url?>"></a>
  </div>
	<div class="col-md-12">
	    <hr>
    </div>
