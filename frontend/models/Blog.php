	...
	if ($this->id) {$date_update = date('Y-m-d H:i:s');}else{$date_update = '';};
        return [	
		   //$capcha,
			...
			//Обновленная строка
            [['text','title', 'email', 'username', 'password2', 'password', 'category', 'region', 'address', 'auction'], 'required'],
            //Обновленная строка
			[['text', 'dir_name','date_del','coordlat','coordlon','address', 'discount_text', 'auk_price_add', 'auk_price_moment'], 'string'],
