...
if(Yii::$app->controller->action->id == 'auction' && Yii::$app->request->get('text')) {
	 $this->params['breadcrumbs'][] = ['label' => 'Поиск товара по значению "'.Yii::$app->request->get('text').'"'];
	 $this->title = 'Поиск товара';
}elseif(Yii::$app->controller->action->id == 'auction') {
...
<?php foreach ($blogs as $one) {?>
  ...
  <? $url = Url::to(['product/auction', 'region'=>$one->regions['url'], 'category'=>$one->categorys['url'], 'url'=>$one->url, 'id'=>$one->id]);?>
	  ...
	  <div class="cat-board-body">
	   ...
	   <h3><a target="_blank" class="cat-bo-href" data-toggle="tooltip" data-placement="top" title="" data-original-title="<?=Yii::$app->userFunctions->substr_user($one->title, 150)?>" href="<?=$url?>"><?=Yii::$app->userFunctions->substr_user($one->title, 50)?></a></h3>
	        <div class="all_price">
	               <? $price_arr = array();
