...
<div class="add-left-body">
 <? if (isset($_GET['save'])) {?> 
    <div class="alert alert-success">
       ...
       <? if(Yii::$app->caches->setting()['moder-shop'] == '0') {?>
	     Он будет опубликован после проверки.
      <?}?>
	  <br>
	  Вы можете создать слайдер на этой странице <a href="<?=Url::to(['/user/slider', 'shop_id'=> $_GET['save']])?>">Создать слайдер</a>
 ...
 <div class="alert alert-info price">Стоимость публикации магазина на период (<?=Yii::$app->caches->setting()['end-shop']?> дней), <?=Yii::$app->caches->setting()['price-shop']?> <i class="fa <?=$rates['text']?>" aria-hidden="true"></i>, сумма будет удержана с вашего личного счета на сайте.</div>
<?php if(Yii::$app->caches->setting()['price-shop'] <= Yii::$app->user->identity->balance) {?>
...
<div class="help-block"></div><label class="control-label" for="shop-delivery">График работы <span class="req_val">*</span></label>
  ...
  <table class="table table-bordered graf-table" style="width: 70%">
	 ...
	 foreach($arr as  $key => $res ) {?>
        <tr class="<?if ($model->grafik_arr[$key]['vih']) {?>opacity<? }?>">
          <td><?=$res?>: </td>
...
<br>
<!--<div class="hr_add"><i class="fa fa-filter" aria-hidden="true"></i> Фото магазина или товара</div><div class="mb-max"><i class="fa fa-exclamation" aria-hidden="true"></i> не более <?=Yii::$app->caches->setting()['max_photo_mb']/1000?> Мб</div>-->
...
<div class="col-md-12">
   <div class="row">
      <div class="alert alert-danger">У Вас недостаточно средств, пополните баланс на <span class="minus"><?=$minus?>  <i class="fa <?=$rates['text']?>" aria-hidden="true"></i></span>, после этого повторите регистрацию магазина. <a href="<?=Url::to(['user/balance'])?>">Пополнить баланс</a></div>
   </div>
</div>
<?php } ?>
<?php } ?>
