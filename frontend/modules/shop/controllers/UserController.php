...
class UserController extends Controller
{
    ...
    public function actionIndex($url)
    {
		...
		$arr = $this->findModelshop($url);
		$this->layout = 'shop_user';
	    $shop = $arr['model'];
		$meta = $arr['meta'];
		$vote = $arr['vote'];
      ...
      if (!Yii::$app->user->isGuest) {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
			if ($model->password) {
				...
				$this->layout = 'shop_user';
             return $this->render('personal',
                compact('model', 'shop', 'price', 'comment', 'vote', 'count_art')
            );
        }
    ...
    ->all();
        ...
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams, Yii::$app->user->id,  $arr['model']['id']);
    ...
    public function actionBalance($url)
    {         
	...
	if (Yii::$app->user->isGuest) {
	   ...
	   $payment = $this->findPayment();
				$this->layout = 'shop_user';
       return $this->render('balance',  compact('payment', 'model', 'shop', 'price', 'comment', 'vote', 'count_art'));
    }
    ...
    public function actionHistory($url)
    {
...
foreach($_GET as $key => $get) {
 ...
 $sliders = $this->findSliders();
	...
	protected function findModelshop($url)
    {
