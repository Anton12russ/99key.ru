...
<div class="form-group field-field-cat ">
<label class="control-label" for="field-name"><?=$res['name']?></label>
<span class="sort-chex"><? $val = array(); foreach(explode("\n",$res['values']) as $key => $value) {?><? if((isset($get['f_'.$res['id'].'_chex_'.$key]) && $get['f_'.$res['id'].'_chex_'.$key] == $key) && ($get['f_'.$res['id'].'_chex_'.$key] != '')) {$val[] =  str_replace("\r", '' , $value); } ?><? }?><? if ($val) {?><?=implode(', ',$val);?><? }else{ ?>Выбрать<? }?></span>

     <ul class="sortmenu" role="menu" aria-labelledby="dLabel" >
        <div class="treug"></div>
...
<div class="form-group field-field-cat ">
<label class="control-label" for="field-name"><?=$res['name']?></label>
<span class="sort-chex"><? $val = array(); foreach(explode("\n",$res['values']) as $key => $value) {?><? if((isset($get['f_'.$res['id'].'_multi_'.$key]) && $get['f_'.$res['id'].'_multi_'.$key] == $key) && ($get['f_'.$res['id'].'_multi_'.$key] != '')) {$val[] =  str_replace("\r", '' , $value); } ?><? }?><? if ($val) {?><?=implode(', ',$val);?><? }else{ ?>Выбрать<? }?></span>
