...
class AjaxController extends Controller
{
 ...
 //Исключение запятой стоящей в конце строки
	...
	$pushs =Yii::$app->userFunctions3->Pushonline();
	if($pushs) {
	    $arr['push'] = array();
		$arr['push']['text'] = $pushs->text;
		$arr['push']['url'] = $pushs->href;
	}
    ...
    { 
	 ...
	 /* $new_date = date_create(date("Y-m-d H:i:s"));
      ...
      $second_date = (date_timestamp_get($new_date))*1000;
		...
		public function actionTimerauction()
    { 
	
		  $this->layout = 'style_none';

		  return $this->render('timerauction');
...
public function actionCopy($id)
 {
	 ...
	 $price_category = $this->findModprice($model->category, $model->region);
		if ($price_category) {
		 ...
		 if($model->auctions) {
			 $model_copy->auction =  $model->auction;
		 }
	  ...
	  //Сохраняем допполя
	   ...
	   if($model->auctions) {
	 	$auction = new BlogAuction();
			$auction->price_add =  $model->auctions['price_add'];
			if($model->auctions['price_moment']) {
			    $auction->price_moment = $model->auctions['price_moment'];
			}
		 ...
		 $date1 = strtotime($model->auctions['date_add']);
		 ...
		 $auk_time = ceil($auk_time);
			...
			$auk_time = $auk_time-1;
			$model->date_del =  $auk_time;
			$model->update();
			$auction->date_end = date('Y-m-d H:i:s', strtotime(' + '.$auk_time.' day'));
			$auction->blog_id = $model_copy->id;
			$auction->user_id = $model->user_id;
			$auction->rates = $model->auctions['rates'];
			$auction->shag = $model->auctions['shag'];
			$auction->save();  
	   }
    ...
    { 
    $redirect = str_replace('https://1tu.ru','',$redirect);
	 $model = new LoginForm();
	...
	public function actionAuctionreserv($id)
    {
        $blog = Blog::find()->andWhere(['id'=>$id])->One();
		//Отменяем все ставки и отсылаем всем сообщения об отмене
				$bets = Bet::find()->where(['blog_id'=> $blog->id])->all();
				...
				if(isset($betarr)) {
					foreach($betarr as $ba) {
						 Yii::$app->functionMail->betdelmailreserv($ba, $blog);
				     }
			    }
		if(Yii::$app->user->id && $blog->auction != 2) {
		  $blog->auction = 2;
		  $blog->reserv_user_id = Yii::$app->user->id;
		  $blog->update();
		  
		
		  //Емэйл сообщение
		  $email_to = $blog->author['email'];
		  $subject = 'Лот зарезервировали';
		  $text = 'Ваше объявление "'.$blog->title.'" было зарезервировано пользователем '.$blog->reservuser['email'];
		  
		  
		  
		 Yii::$app->functionMail->reservlotauthor($blog);

		$text = 'Ваше объявление "'.$blog->title.'" было зарезервировано пользователем "'.$blog->reservuser['username'].'"';
		  
		Yii::$app->userFunctions3->Push($blog->user_id, $text, 'https://1tu.ru/user/auction');
		...
		//////////////////////////////////////////////////////
foreach( $blog->blogField as $field) {
    if($field['field'] == '475') {
	    $phone = $field['value'];
    }
}
  $address = $blog->coord['text'];
...
$url = Url::to(['/blog/one', 'region'=>$blog->regions['url'], 'category'=>$blog->categorys['url'], 'url'=>$blog->url, 'id'=>$blog->id, 'general' => 'true']);
Yii::$app->functionMail->resevinfo($blog, $info, $url);
		...
		$this->layout = 'style_none';
		  return $this->render('auctionreserv', [
            'model' => $blog,
        ]);
	}
	...
	public function actionAuctionhistory($id)
    { 
        $model = Bet::find()->andWhere(['blog_id'=>$id])->orderBy(['id' => SORT_DESC])->All();
		if($model) {
			
		    $this->layout = 'style_none';
            return $this->render('auctionhistory', [
                'model' => $model,
            ]);
		 }
	}
	...
	public function actionReservdel($id)
    { 
        $model = Blog::find()->andWhere(['id'=>$id, 'user_id' => Yii::$app->user->id])->One();

		if($model->reserv_user_id) {
	       Yii::$app->functionMail->reservdelauthor($model);
		}
		if($model) {

		  $model->auction = 1;
		  $model->reserv_user_id = NULL;
		  $model->update();
		 }
	}
		...
		public function actionReservdeluser($id)
    { 
        $model = Blog::find()->andWhere(['id'=>$id, 'reserv_user_id' => Yii::$app->user->id])->One();
		if($model) {
		  Yii::$app->functionMail->reservdel($model);
		  $model->auction = 1;
		  $model->reserv_user_id = NULL;
		  $model->update();
		 }
	...
	public function actionAuctiondel($id)
    {
	...
	public function actionPriceajax($id, $price)
    {
      $model_field = BlogField::find()->Where(['message' => $id, 'field' => 481])->One();
	  $model_field->value = $price;
	  $model_field->update();
	  return $price;
	}
}
