...
//Список объявлений на главной
  ...
  public function actionAll($array = false)
  {
  ...
  if(!isset($arr)) {
    ...
    $query = Blog::find()->andWhere(['id' => $arr, 'status_id' => 1, 'active' => 1]);
...
public function actionCoord()
	...
	if(isset($get['category'])) {
	   ...
	   if (isset($region)) {
         ...
         $params = ['status_id'=>1, 'blog.active'=>1, 'category'=>$cat_all, 'region' => $reg_all];
...
foreach($price_rt_arr as $res) {
  ...
  foreach ($price_arr as $res) {
    ...
    if(isset($get['note'])) {
  	...
  	foreach(Yii::$app->caches->field() as $fields) {
		if($fields['type'] == 'j') {
			$coord = $fields['id'];
			break;
		}
           ...
           $coordid = 'bd_'.$coord;
	        $sql->LeftJoin('blog_field  '.$coordid.'', ''.$coordid.'.message = blog.id')
	       ->andWhere($coordid.'.field = '.(int)$coord.'');
...
/*------------------------------------------------------------------------------------------*/ 
	...
	//var_dump( $sql->prepare(\Yii::$app->db->queryBuilder)->createCommand()->rawSql);
...
foreach($query as $res) {
	foreach($res['blogField'] as $fieldres ) {
      if($fieldres['field'] == $coord) {
		  $coordin = explode (', ',$fieldres['value']);
	  }
	}
